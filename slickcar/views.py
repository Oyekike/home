...
from slickcar.forms import ContactForm
from . models import * 
...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new = user.first_name.title()
            messages.success(request, f'Dear {new} your profile update generated the following errors: {form.errors}')
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Dear {new} your password has been changed successfully')
            return redirect('profile')
        else:
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_0a037fbbf8848d3685e050aad3f08b751a4237af' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://54.152.116.243/callback' #payment confirmation page
        ...
        total = float(request.POST['total']) * 100 #total amount to be changed from customer card
...
def search(request):
    if request.method == 'POST':
        ...
        searched = Q(Q(name__icontains=item)| Q(model__icontains=item)| Q(year__icontains=item))   
